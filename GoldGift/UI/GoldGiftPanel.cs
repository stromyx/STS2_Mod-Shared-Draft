...
public static class GoldGiftPanel
{
    ...
    private static void BuildUI(PanelContainer panel)
    {
        ...
        _titleLabel.Text = GoldGiftConfig.DebugMode
            ? "🎁 " + GoldGiftLocalization.Get("title_debug")
            : "🎁 " + GoldGiftLocalization.Get("title");
        ...
        _goldLabel.Text = GoldGiftLocalization.Get("your_gold", "---");
        ...
        selectLabel.Text = GoldGiftLocalization.Get("select_player");
        ...
        amountLabel.Text = GoldGiftLocalization.Get("amount");
        ...
        customLabel.Text = GoldGiftLocalization.Get("custom");
        ...
        _sendButton.Text = GoldGiftLocalization.Get("send");
    ...
    public static void Toggle()
    {
        if (_panel == null || !GodotObject.IsInstanceValid(_panel))
        {
            ModEntry.Logger.Info("[GoldGift] Toggle called but panel is null or invalid.");
            return;
        }

        _panel.Visible = !_panel.Visible;
        ModEntry.Logger.Info($"[GoldGift] Panel toggled. Visible={_panel.Visible}");
        if (_panel.Visible)
        {
            Refresh();
        }
    ...
    private static void RefreshGoldDisplay()
    {
        ...
        if (_goldLabel != null)
        {
            _goldLabel.Text = GoldGiftLocalization.Get("your_gold", gold);
        }
    }
    ...
    private static void RefreshPlayerList()
    {
        ...
        if (players.Count == 0)
        {
            ...
            noPlayersLabel.Text = GoldGiftLocalization.Get("no_players");
    ...
    private static void OnSendPressed()
    {
        ...
        if (amount <= 0)
        {
            SetStatus(GoldGiftLocalization.Get("invalid_amount"), new Color(1, 0.4f, 0.4f));
            return;
        }
        ...
        if (localGold <= 0)
        {
            SetStatus(GoldGiftLocalization.Get("cannot_find_char"), new Color(1, 0.4f, 0.4f));
            return;
        }

        if (localGold < amount)
        {
            SetStatus(GoldGiftLocalization.Get("not_enough", localGold),
                new Color(1, 0.4f, 0.4f));
            return;
        }

        // Disable send button to prevent double-click
        if (_sendButton != null) _sendButton.Disabled = true;

        bool success = GoldGiftNetworkHandler.SendGoldGift(_selectedSlotId, amount);

        if (success)
        {
            SetStatus(GoldGiftLocalization.Get("success", amount), new Color(0.4f, 1, 0.4f));

            // Delay refresh slightly to allow the network action to execute
            // and update Gold values on this client
            var tree = _panel.GetTree();
            if (tree != null)
            {
                var refreshTimer = tree.CreateTimer(0.3);
                ...
                var closeTimer = tree.CreateTimer(1.5);
                closeTimer.Timeout += () =>
                {
                    if (_panel != null && GodotObject.IsInstanceValid(_panel))
        ...
        {
            SetStatus(GoldGiftLocalization.Get("failed"), new Color(1, 0.4f, 0.4f));
            if (_sendButton != null) _sendButton.Disabled = false;
        }
    }
